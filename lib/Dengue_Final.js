...
function updateSurface(){
  ...
  // allgraphs.exit().remove()
	...
	//Keenan: export the stats in html text form rather than d3 as d3 exports it as svg element
/*
...
function updateMosquito(mosquitoIndex){
	...
	switch(state){
		case FLYING:
			// kill the mosquito after at most 14 simulation steps
			if (mosquito.timeAdmitted >= 14){
...
function updatePerson(personIndex){
	...
	switch(state){
		case SICK:
			...
			// person recovers from dengue after at most 17 simulation steps
			if (person.timeInfected > 6+Math.random()*11){
...
function updateStats(){
	// update the output statistics at each simstep 76
...
function simStep(){
	...
	if (isRunning){ //the isRunning variable is toggled by toggleSimStep
		...
		// 10 new mosquitoes will be spawned periodically every 80 sim step (2x lifespan of mosquito)
		if (currentTime%80 == 0){
