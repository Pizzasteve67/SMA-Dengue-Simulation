...
// human density
var humanDensityZone1 = 0.2;
var humanDensityZone2 = 0.4;
var humanDensityZone3 = 0.6;
var humanDensityZone4 = 1;

// trap density
var trapDensityZone1 = 0.05;
var trapDensityZone2 = 0.05;
var trapDensityZone3 = 0.05;
var trapDensityZone4 = 0.05;

// water sources (for reproduction of mosquito) density
var waterDensityZone1 = 0.1;
var waterDensityZone2 = 0.1;
var waterDensityZone3 = 0.1;
var waterDensityZone4 = 0.2;
...
function generateAll(rowList, colList){
	...
	for (i = 0; i < rowList.length; i++){
		for (j = 0; j < colList.length; j++){
			var newindex ={"location":{"row":rowList[i], "col":colList[j]}, "id": String(rowList[i])+","+String(colList[j])};
...
// The probability of a mosquito spawning at a water source (probArrival); The probability of a mosquito dying (probDeparture).
var probArrival = 0.95;
var probDeparture = 0.05;
...
function addMosquito(currentrow,currentcol, initialize = true){
	// Mosquitoes are dynamic agents: they are spawned in the city, commute and then die
		...
		case FLYING:
			if (hasArrived){
				if (Math.random()<probDeparture){
                    ...
                    //Mosquito is still commuting// specifies a new target within the zone (cannot be a building)
                    var targetrow=Math.floor(Math.random() * ((nrow+srow) - srow) +srow);
                    var targetcol=Math.floor(Math.random() * ((ncol+scol) - scol) +scol);
