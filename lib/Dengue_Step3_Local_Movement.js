...
function addMosquito(currentrow,currentcol, initialize = true, zone){
	// Mosquitoes are dynamic agents: they are spawned in zone 3/4 randomly, commute and then die
	...
	if (initialize){
		if(Math.random()<0.5){
			...
			var newmosquito = {"id":id++,"type":"I","location":{"row":homerow,"col":homecol},
			"target":{"row":targetrow,"col":targetcol},"state":FLYING,"timeAdmitted":0, "zone":'zone4'};
			//console.log(newmosquito)
			mosquitoes.push(newmosquito);
		}
...
function updateMosquito(mosquitoIndex){
	...
	switch(state){
		case FLYING:
			...
			if (mosquito.timeAdmitted > 40+Math.random()*5){
